...
use chrono::{Datelike, Duration, Local, NaiveDate, Weekday as CWeekday};
...
fn cell_size(width: f32, height: f32, rows: u32) -> (f32, f32) {
    ...
    let avail_h = height - TOOLBAR_BTN_Y - TOOLBAR_BTN_SIZE - 8.0 - CAL_HEADER_H - SIDE_PAD;
    let cell_h = ((avail_h - (rows as f32 - 1.0) * CELL_GAP) / rows as f32).max(40.0);
    (cell_w, cell_h)
}

/// Grid origin (top-left of the first cell slot).
fn grid_origin(_height: f32) -> (f32, f32) {
    let top_content = TOOLBAR_BTN_Y + TOOLBAR_BTN_SIZE + 8.0;
    let gx = SIDE_PAD;
    let gy = top_content + CAL_HEADER_H;
    (gx, gy)
}
...
fn draw_month_grid(
    ...
    for day in 1..=dim {
        ...
        let sched_hours = plan.default_schedule.hours_on(wd);
        let bg = if let Some(override_h) = plan.calendar.get(date) {
            if override_h == 0.0 {
                CAL_HOLIDAY_BG
            } else {
                CAL_PARTIAL_BG
        ...
        if let Some(h) = plan.calendar.get(date) {
            let h_str = format!("{h:.1}h");
        ...
        if state.editing_date == Some(date) {
            draw_edit_popup(canvas, rect, state, cache, &paint);
        }
    }
}
...
fn draw_edit_popup(
    ...
    // Hint
    if let Some(blob) = TextBlob::new("↵ set  esc cancel", &cache.small_font) {
        let bounds = blob.bounds();
        let (_, m) = cache.small_font.metrics();
...
/// Hit-test a day cell from mouse position. Returns (day, date) if hit.
pub fn hit_test_day(
    x: f32,
    y: f32,
    width: f32,
