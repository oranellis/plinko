...
use plinko_shared::protocol::{MilestonePatch, PlanRequest, TaskPatch, apply_task_patch};
use std::cell::Cell;
...
impl TaskFormWindow {
    ...
    pub fn from_task(task: &Task, plan: &plinko_shared::data::Plan) -> Self {
        ...
        Self {
            ...
            hovered_dep_plus: false,
            dep_error: false,
            pre_generated_id: task.id,
            dependents: {
                let node_id = NodeId::Task(task.id);
    ...
    /// Returns the task ID to use for this form: the actual task ID in edit mode,
    /// or the pre-generated ID in new mode (used for fwd dependent cycle-checking
    /// and for wiring up dependents after the task is created).
    fn effective_task_id(&self) -> TaskId {
        match self.mode {
            Mode::Edit(id) => id,
    ...
    fn try_submit(&mut self, plan: &Plan, sender: &PlanRequestSender) -> FloatingWindowOutcome {
        ...
        let sched_result: Result<(), String> = match self.mode {
            Mode::New => (|| -> Result<(), String> {
                ...
                // Apply dependent changes for new tasks too
                let current_node = NodeId::Task(task_id);
            ...
            Mode::Edit(id) => {
                ...
                // cycle-check inside apply_task_patch sees the updated graph.
                let current_node = NodeId::Task(id);
        ...
        match self.mode {
            Mode::New => {
                ...
                // Wire up forward dependents: add the new task as a dependency of each
                let current_node = NodeId::Task(task_id);
                for (dep_node, lag) in &new_dependents {
                    match dep_node {
                        NodeId::Task(t_id) => {
            ...
            Mode::Edit(id) => {
                ...
                // sees the updated graph before the edited task's new backward deps arrive.
                let current_node = NodeId::Task(id);
                let new_dep_node_set: std::collections::HashSet<NodeId> =
                    new_dependents.iter().map(|(n, _)| *n).collect();
                let old_dependents_map = plan.build_dependents_map();
...
fn draw_fwd_dropdown(
    ...
    // Tasks (excluding self and those that would create a cycle)
    let mut task_items: Vec<(NodeId, String)> = plan
        .tasks
        .iter()
        .filter(|(id, t)| {
    ...
    // Milestones (excluding self and those that would create a cycle)
    let mut ms_items: Vec<(NodeId, String)> = plan
        .milestones
        .iter()
        .filter(|(id, m)| {
...
impl FloatingWindow for TaskFormWindow {
    fn render(&self, canvas: &Canvas, width: f32, height: f32, cache: &RenderCache, plan: &Plan) {
            ...
            && dep_idx < self.dependents.len()
        {
            ...
            let dd_rect = TaskFormWindow::fwd_dropdown_rect(adjusted_fwd_list, dep_idx, panel);
            let this_node = NodeId::Task(self.effective_task_id());
            draw_fwd_dropdown(
                canvas,
                dd_rect,
            ...
            && dep_idx < self.dependents.len()
        {
...

            {
                ...
                for dep in &mut self.dependents {
                    if dep.hovered_target || dep.hovered_remove {
                        changed = true;
                    }
                    dep.hovered_target = false;
                    dep.hovered_remove = false;
        ...
        // Fwd list interactions
        {
            ...
            if fwd_list2.contains(pt_form) {
                ...
                for abs in 0..self.dependents.len() {
                    if TaskFormWindow::fwd_remove_rect(fwd_list2, abs).contains(pt_fwd) {
                    ...
                    if TaskFormWindow::fwd_target_rect(fwd_list2, abs).contains(pt_fwd) {
                        self.open_fwd_dropdown(abs);
                        return FloatingWindowOutcome::dirty(DirtyRegion::PageOnly);
                    }
                    if TaskFormWindow::fwd_lag_rect(fwd_list2, abs).contains(pt_fwd) {
                        ...
                        self.description.focused = false;
                        self.duration.focused = false;
                        let lag_rect = TaskFormWindow::fwd_lag_rect(fwd_list2, abs);
